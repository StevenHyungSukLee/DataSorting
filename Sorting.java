...
public class Sorting {
	...
	public static void BubbleSort(long[] a) {
		...
		for(int i = 0; i < a.length; i++){
			for(int j = 0; j < a.length -1; j++){
				if(a[j] > a[j+1]){ //if the left element is larger than right, swap two elements' indices

					exch(a, j, j+1); //helper method that swapping a[i] and a[j]
				}
			}
		}
	...
	public static void MergeSortNonRec(long[] a) {
		...
		for(int k = 1; k < a.length; k = 2*k){//it starts merging subarrays of size 1 (k = 1), then a pass of 2 by 2, 4 by 4,... until it reaches to the original array length
			for(int i=lo; i < hi; i += 2*k){
				int start = i;
				int mid = i+k-1;
				int end = Math.min(i+2*k-1, hi);
